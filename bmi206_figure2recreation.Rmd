...
**0. Each individual implementation of the method done by each of us team members were able to obtain the same figure, across different languages as well (e.g. Python, R).**

**1. Permuting the edges in the adjacency matrix (keeping nodes and number of edges the same, just randomizing the connections)**

Seemingly metabolic networks are *scale-free* networks. If we permute the adjacency matrix, we turn the graph into a random network, and we see that the power law trend for the degree distribution breaks down.
...
# also not sure it's relevant to recalculate permutations of gammas, since when we randomize
# the network, the distribution of degrees should no longer follow the power law
...
**2. Does distribution of degrees among all reaction networks combined follow the same power law trend?**

```{r}
combined_deg_df = data.frame(Reaction=character(0),
                         in_degree=numeric(0),
                         out_degree=numeric(0),
                         total_degree=numeric(0)
                         )
...
for (model in models) {
  adj_df = file_to_adj_df(file_path, model)
  deg_df = calculate_degrees(adj_df)
  combined_deg_df = rbind(combined_deg_df, deg_df)
}

combined_prob_df = calculate_deg_probs(combined_deg_df)

print(plot_deg_probs(combined_prob_df, "All Models", palette))
```
